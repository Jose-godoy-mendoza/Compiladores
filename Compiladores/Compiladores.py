...
def ErrorHandling(Line, Match, LineCount):
    ...
    if "PR( if )" in Line:
        ...
        if "OP( ( )" in Line[1] and "ID" in Line[2] and Line[3] in Operators and ("OP( ) )" in Line[Position] or "OP( ) )" in Line[Position-1]):
            ...
            Message = "ERROR SINTACTICO: if mal declarado en la linea: ", LineCount
            LowerTextbox.insert('end-1c', Message)
            LowerTextbox.insert('end-1c', "\n")
    elif "PR( for )" in Line:
        ...
        if len(temp)==5:
            ...
            if  ("@" in temp[0] or "@" in temp[1] or "@" in temp[2] or ("@" in temp[3] and "ID" not in temp[3]) or "@" in temp[4]) and "STR" not in temp[3]:
                ...
                # if '"' in CheckString
                    ...
                    ID = ""
                    Message = "Falta el ; al final de la linea: ", Row_index+1
                    LowerTextbox.insert('end-1c', Message)
                    LowerTextbox.insert('end-1c', "\n")
                else:
                    ID = ""
                ...
                #temp = []
        elif "PR" in temp[0] and len(temp)>1 or "@" in temp[0]:
            if "ID" in temp[1] and len(temp)>2 or "@" in temp[1]: 
                if "OP" in temp[2] and len(temp) != 5 or "@" in temp[2]:
...
def Clasify_tokens(Token):
    ...
    #['assert', 'bool', 'break', 'case', 'catch', 'char', 'class', 'continue', 'default', 'do', 'double', 'else', 'enum', 'extends',
              #'false', 'finally', 'float', 'for', 'if', 'implements', 'import', 'instanceof', 'int', 'interface', 'main', 'new', 'null',
              #'package', 'private', 'protected', 'public', 'return', 'static', 'string', 'switch', 'this', 'throw', 'throws', 'true',
              #'try', 'var', 'void', 'while']
